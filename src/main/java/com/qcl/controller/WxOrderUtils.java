...
public class WxOrderUtils {
    ...
    //用户催单
    public void cuiDan(Integer orderId) {
        try {
...
//    }
    ...
    public void exportOrderToExcel(HttpServletResponse response) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            dataList[i][3] = "" + orderRoot.getOrderAmount();
            dataList[i][4] = EnumUtil.getByCode(orderRoot.getOrderStatus(), OrderStatusEnum.class).getMessage();
            dataList[i][5] = "" + orderRoot.getCreateTime();
        }
